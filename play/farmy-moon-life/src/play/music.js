...
export function buildGraph(ctx, arr, score) {
  ...
  for (const track of arr.tracks) {
    ...
    for (const n of track.notes) {
      ...
      const g = envelope(ctx, at, n.gain, attack, decay, holdUntil);
      g.connect(bus);
...
export async function renderArrangement(arr, {
  ...
  const headroom = Math.ceil(rate * 2.5);
  const ctx = new Offline(1, loop + headroom, rate);
  buildGraph(ctx, arr, score);
